...
const FeaturedProducts = () => {
  return (
    <div className="max-w-7xl mx-auto px-4 py-8">
      ...
      <div className="flex gap-4 overflow-x-auto pb-4 scrollbar-hide cursor-pointer">
        {limitedProducts.map((product, index) => (
        <Link to={`/product/${index}`}>
          <div
            key={product.asin || index}
            className="min-w-[230px] max-w-[230px] min-h-[310px] max-h-[310px] flex-shrink-0 border rounded-lg shadow hover:shadow-lg p-4 bg-white"
          >
            <img
              src={product.imgUrl}
              alt={product.title}
